...
/**
 * TimelineProgress - Overall journey visualization
 *
 * Horizontal path with milestone dots, house at end.
 * Dynamic props for current step highlighting.
 * Conveys: "Your journey at a glance"
 */

const DEFAULT_STEPS = [
  { id: 1, label: "Pre-Approval", icon: "$" },
  { id: 2, label: "Find Agent", icon: "👤" },
  { id: 3, label: "Search", icon: "🔍" },
  { id: 4, label: "Offer", icon: "📝" },
  { id: 5, label: "Inspection", icon: "✓" },
  { id: 6, label: "Closing", icon: "🔑" },
  { id: 7, label: "Move In", icon: "🏠" },
];
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      >
        ...
        {/* House icon at end */}
        <g
          filter={currentStep === totalSteps ? "url(#activeGlow)" : undefined}
          style={{
            opacity: mounted ? 1 : 0,
